...
public class ClassUtil {
	...
	public static List<Class> getAllDao() {
		...
		// System.out.println(new
		// File("C:\\Users\\Administrator\\Workspaces\\MyEclipse
		// 10\\FatHat\\bin\\org\\fathat\\dao\\EnterpriseDao.class").exists());==>true
		String basePackage = null;
		basePackage = PropertiesUtil.getProperties("db.properties").getProperty("dao.basePackage");
		File file = null;
		try {
			file = new File(URLDecoder.decode(
					ClassUtil.class.getClassLoader().getResource(basePackage.replace(".", "\\")).getPath(), "utf-8"));
			...
			if (currentFile.isDirectory()) {
				...
				for (File _file : files) {
					try {
						...
						Class<?> clazz = PropertiesUtil.class.getClassLoader()
								.loadClass(currentPrefix + "." + _file.getName().split("\\.")[0]);
						classList.add(clazz);
				...
				try {
					Class<?> clazz = PropertiesUtil.class.getClassLoader()
							.loadClass(currentPrefix + "." + file.getName().split("\\.")[0]);
					classList.add(clazz);
				} catch (ClassNotFoundException e) {
					e.printStackTrace();
				}
	...
	public static boolean isBasicType(Class returnType) {
		return Short.class == returnType || Integer.class == returnType || Float.class == returnType
				|| Double.class == returnType || Long.class == returnType || Character.class == returnType
				|| String.class == returnType || Boolean.class == returnType || Date.class == returnType;
	}
